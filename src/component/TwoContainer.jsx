...
export default function TwoContainer() {
  ...
  useEffect(()=>{
    if(apiData?.images){
      setData(apiData.images[0].skills);
    }else{
      console.log(apiData)
    }
  },[apiData])
        ...
        <div className='w-full flex flex-wrap m-5 items-center justify-start'>
          {
            data?.map((e) => {
              return (
                <div key={e.img} className='w-3/12 transition-transform duration-300 transform hover:scale-110 cursor-pointer my-7'>
        ...
        <div className='lg:w-10/12 w-11/12 mx-auto'>
          ...
          <div className='my-8 items-center flex flex-col md:flex-row w-full justify-between'>
            <a href="https://github.com/aryanmalla19" target='_blank' rel="noreferrer">
              <button className="flex w-[200px] gap-3 cursor-pointer text-white font-semibold bg-gradient-to-r from-gray-800 to-black px-7 py-3 rounded-full border border-gray-600 hover:scale-105 duration-200 hover:border-gray-800 hover:from-black hover:to-gray-900">
