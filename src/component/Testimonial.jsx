    ...
    return (
        <div className='flex lg:flex-row flex-col my-14 justify-around'>
            ...
            <div className='lg:w-7/12 w-full'>
                <div className='w-full overflow-hidden relative flex pb-20'>
                    ...
                    {data? (data.map((e, index) => (
                        <div key={index} className='min-w-full flex'>
                            <div className='transition-transform duration-500 ease-out' style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
                                <img src={e.img} className='rounded-full mx-auto w-[100px] mb-4 h-[100px] md:w-[150px] md:h-[150px]' alt="" />
                                <h2 className='text-xl text-main font-semibold text-center'>{e.name}</h2>
                                <h4 className='text-base text-main text-center my-2'>{e.place}</h4>
                                <p className='lg:my-7 my-2 text-sm text-main text-center'>{e.saying}</p>
                            </div>
                        </div>
                    ))):("")
                }
